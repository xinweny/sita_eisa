...
<<<<<<< HEAD
  ...
  if (!grepl("WBGene", genes[1], fixed=TRUE)) {
...
ttEx <- topTags(lrtEx, n=nrow(y), sort.by="none")
ttIn <- topTags(lrtIn, n=nrow(y), sort.by="none")
...
MAplot <- ggplot(ttEx$table, aes(x=logCPM, y=logFC)) +
  geom_point(color="lightgrey") +
  geom_point(data= ttEx$table %>% filter(FDR < 0.05),
             color="red") +
  labs(title=glue("{gse}: {unique(cond)[1]} vs. {unique(cond)[2]} (Exonic)"),
...
# Save output
png(glue("processed/{gse}_eisaMAplot_{selectConditions[1]}.{selectConditions[2]}_exonic.png"))
print(MAplot)
dev.off()
...
MAplot <- ggplot(ttIn$table, aes(x=logCPM, y=logFC)) +
  geom_point(color="lightgrey") +
  geom_point(data= ttIn$table %>% filter(FDR < 0.05),
             color="red") +
  labs(title=glue("{gse}: {unique(cond)[1]} vs. {unique(cond)[2]} (Intronic)"),
...
# Save output
png(glue("processed/{gse}_eisaMAplot_{selectConditions[1]}.{selectConditions[2]}_intronic.png"))
print(MAplot)
dev.off()
